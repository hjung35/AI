...
class Maze:
    ...
    def __init__(self, input_map, offsets, granularity):        
        ...
        # a little change here to ndarray 
        self.__dimensions = np.array(input_map).shape       
        self.__map = input_map

        if len(self.__dimensions) == 1:
    ...
    # check the validity of the movement, same format using beta as standard, little modification here and there just like the above
    def isValidMove(self, alpha, beta=None, gamma=None):
        ...
        #this was the original case with 2D given two angles         
        elif beta is not None:    
            x, y = angleToIdx((alpha, beta), self.offsets, self.granularity)
            return x >= 0 and x < self.getDimensions()[ALPHA] and \
                y >= 0 and y < self.getDimensions()[BETA] and \
    ...
    # Returns all neighbors list given the condition of existence of alpha beta and gamma        
